...
order_call <- data.frame(order_call(tqqq, expiry_date, strike_price_call, mid_market_call, num_contracts))

# Place the order for the put spread

order_put <- data.frame(order_put(sqqq, expiry_date, strike_price_put, mid_market_put, num_contracts))
...
close_position <- function() {
  if (weekdays(Sys.Date()) == "Friday" & weekdays(as.Date(expiry_date)) == "Friday") {
    order_call_close <- data.frame(close_order_call(tqqq, expiry_date))
    order_put_close <- data.frame(close_order_put(sqqq, expiry_date))
  } else if (Sys.Date() == as.Date(expiry_date)) {
    order_call_close <- data.frame(close_order_call(tqqq, expiry_date))
    order_put_close <- data.frame(close_order_put(sqqq, expiry_date))
  }
}
